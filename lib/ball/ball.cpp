...
void ball::setTokensFromPixelCoordinates(uint8_t x, uint8_t y) {
    ...
    uint8_t col = getLeftColFromPixelCoordinate(x);
    uint8_t row = getBottomRowFromPixelCoordinate(y);

    x = x - getPixelOffsetFromCol(col);
    y = y - getPixelOffsetFromRow(row);
...
uint8_t ball::getLeftColFromPixelCoordinate(uint8_t x) {
    if (x <= 9) {
        return 0;
    }
    if (x > xMax) {
...
uint8_t ball::getBottomRowFromPixelCoordinate(uint8_t y) {
    if (y <= 15) {
        return 3;
    }
    if (y <= 23) {
...
uint8_t ball::getPixelOffsetFromCol(uint8_t col) {
    return (col * (HD44780::horizontalPixelsPerCharacter + HD44780::pixelsBetweenCharacters));
}

uint8_t ball::getPixelOffsetFromRow(uint8_t row) {
    return ((3-row) * (HD44780::verticalPixelsPerCharacter + HD44780::pixelsBetweenCharacters));
}
